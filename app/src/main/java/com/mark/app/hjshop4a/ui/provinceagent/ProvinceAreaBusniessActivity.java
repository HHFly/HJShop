...
public class ProvinceAreaBusniessActivity extends BaseActivity implements OnRefreshLoadmoreListener{
    ...
    private void requestData(final int curPage) {
        ProvinceAB areaBusniess =new ProvinceAB();
        ArrayList<ProvinceAB> data =new ArrayList<>();
    ...
    public void initRvAdapter(ArrayList<ProvinceAB> data, boolean isRefresh) {

        if (mAdapter == null) {
            mAdapter = new ProvinceAreaBusniessAdapter(data.get(0),data);
