    ...
    public void setDateList(List<DateBean> dates, int currentMonthDays) {
        ...
        for (int i = 0; i < dates.size(); i++) {
            ...
            if (item_layout != 0 && calendarViewAdapter != null) {
                ...
                view = LayoutInflater.from(mContext).inflate(com.mark.app.hjshop4a.R.layout.item_month_layout, null);
                    ...
                    if (date.getType() == 1) {//点击当月
                        if (mAttrsBean.getChooseType() == 1 && chooseListener != null) {//多选的情况
                            ...
                            if (chooseDays.contains(day)) {
                                ...
                                calendarView.mDates.remove(date);
                                flag = false;
//                                calendarView.setChooseDate(day, flag, -1);
                                chooseListener.onMultiChoose(v, date, flag,calendarView.mDates);
                            } else {
                                if (chooseDays.size()<2&& calendarView.mDates.size()<2) {
                                    setDayColor(v, COLOR_SET);
                                    chooseDays.add(day);
                                    calendarView.mDates.add(date);
                                    flag = true;
//                                    calendarView.setChooseDate(day, flag, -1);
                                    chooseListener.onMultiChoose(v, date, flag,calendarView.mDates);
                                }
